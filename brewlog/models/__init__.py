...
def breweries(public_only=True, extra_user=None):
    ...
    if public_only:
        if extra_user:
            query = query.join(BrewerProfile).filter(or_(BrewerProfile.is_public==True, BrewerProfile.id==extra_user.id))  # noqa
        else:
...
def brews(public_only=True, extra_user=None):
    ...
    if public_only:
        ...
        if extra_user:
            query = query.filter(or_(BrewerProfile.id==extra_user.id, and_(BrewerProfile.is_public==True, Brew.is_public==True)))  # noqa
        else:
