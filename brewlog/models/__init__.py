...
def latest_brews(ordering, limit=5, public_only=False, extra_user=None):
    return brews(public_only, extra_user).order_by(db.desc(ordering)).limit(limit).all()
...
def latest_tasting_notes(ordering, limit=5, public_only=False, extra_user=None):
    return tasting_notes(public_only, extra_user).order_by(db.desc(ordering)).limit(limit).all()
